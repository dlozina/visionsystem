...
public partial class HDevelopExport
{
  ...
  private void RunPick(bool picktrigger)
  {
    ...
    {
      ho_Image.Dispose();
      HOperatorSet.ReadImage(out ho_Image, ("D:/Moji Projekti/Vision_System_OMKO/App/VisionApp/Vizijski_Omco_WPF/PickImage/image_No3_"+(hv_Index-1))+".ima.tif");      HOperatorSet.GetImageSize(ho_Image, out hv_Width, out hv_Height);
    ...
    //nastavi dalje ako nema nista na paleti
    ...
    if ((int)(new HTuple(hv_Length.TupleGreater(0))) != 0)
    {
      ...
      HOperatorSet.PaintRegion(ho_FOV_, ho_ImageRectified, out ho_ImageRectified1, 
          0, "fill");
      {
      HObject ExpTmpOutVar_0;
      ...
      for (hv_k=1; hv_k.Continue(end_val159, step_val159); hv_k = hv_k.TupleAdd(step_val159))
      {
        if (hv_x_ == null)
          hv_x_ = new HTuple();
        hv_x_[hv_k-1] = ((hv_Row1.TupleSelect(hv_INDEX.TupleSelect(hv_k-1)))+(hv_Row2.TupleSelect(
            hv_INDEX.TupleSelect(hv_k-1))))/2;
        if (hv_y_ == null)
          hv_y_ = new HTuple();
        hv_y_[hv_k-1] = ((hv_Column1.TupleSelect(hv_INDEX.TupleSelect(hv_k-1)))+(hv_Column2.TupleSelect(
            hv_INDEX.TupleSelect(hv_k-1))))/2;
        if (hv_w_ == null)
          hv_w_ = new HTuple();
      ...
      {
        hv_j = hv_i.Clone();
        while ((int)(new HTuple(((hv_w_.TupleSelect(hv_j-1))).TupleGreater(hv_w_.TupleSelect(
            hv_j)))) != 0)
        {
      ...
      HOperatorSet.ImagePointsToWorldPlane(hv_CamParamOut, hv_WorldPose, hv_x_.TupleSelect(
          0), hv_y_.TupleSelect(0), "mm", out hv_X, out hv_Y);
    ...
    HOperatorSet.GenCircleContourXld(out ho_ContCircle, hv_Height/2, hv_Width/2, 
        100, 0, 6.28318, "positive", 10);
